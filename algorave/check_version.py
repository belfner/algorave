"""Module for checking and comparing algorave package versions.
...
from algorave import __version__ as current_version
...
def fetch_version_info() -> str:
    """Fetch version information from PyPI for algorave package.

    This function retrieves JSON data from PyPI containing information about
    the latest available version of algorave. It handles network errors
    ...
    """
    opener = get_opener()
    url = "https://pypi.org/pypi/algorave/json"
    try:
...
def check_for_updates() -> None:
    """Check if a newer version of algorave is available on PyPI.
    ...
    try:
        ...
        if latest_version:
            ...
            if compare_versions(latest_parts, current_parts):
                warn(
                    f"A new version of Algorave is available: {latest_version!r} (you have {current_version!r}). "
                    "Upgrade using: pip install -U algorave. "
                    "To disable automatic update checks, set the environment variable NO_ALGORAVE_UPDATE to 1.",
                    UserWarning,
                    stacklevel=2,
                )
